...
struct process_callbacks : ex::handler {
  ...
  template <typename Executor> void on_setup(Executor &exec) const {}

  template <typename Executor> void on_success(Executor &exec) const {
...
public:
  ProcessHandler(fs::path executable_path = "", string_ arguments = "",
                 fs::path log_path = "", std::shared_ptr<App> app = nullptr);
